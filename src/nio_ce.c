...
bool nio_scrbuf_init(void)
{
    gfx_Begin();
    return true;
}

void nio_scrbuf_clear(void)
{
    gfx_ZeroScreen();
}

void nio_scrbuf_free(void)
{
    gfx_End();
}
...
unsigned char nio_ascii_get(uint8_t* adaptive_cursor_state)
{
    ...
    switch (key)
    {
        ...
        case sk_Graph : return ' '; // for convenience
        case sk_Vars  : return '#';
        case sk_Yequ  : return '=';
