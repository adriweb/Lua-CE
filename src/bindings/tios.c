...
static int real_new(lua_State* L) {
    real_t* real = calloc(1, sizeof(real_t));
...
{
    char buf[15];
    real_t* real = luacs_checkobject(L, 1, "real_t");
...
{
    free((real_t*)luacs_checkobject(L, 1, "real_t"));
    return 0;
}
...
static int getAns(lua_State *L) {
    ...
    switch (type) {
        ...
        case OS_TYPE_STR:
            lua_pushlstring(L, ((string_t*)ans)->data, ((string_t*)ans)->len);
            return 1;
