...
static int sprite_new(lua_State* L) {
    ...
    gfx_sprite_t* sprite = gfx_MallocSprite(width, height);
    luacs_newobject(L, "gfx_sprite_t", sprite);
    return 1;
}
static int sprite__gc(lua_State *L) {
    free((gfx_sprite_t*)luacs_checkobject(L, 1, "gfx_sprite_t"));
    return 0;
}
static int sprite_draw(lua_State *L) {
    const gfx_sprite_t* sprite = luacs_checkobject(L, 1, "gfx_sprite_t");
